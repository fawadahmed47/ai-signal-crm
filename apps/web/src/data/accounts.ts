...
export async function getAccountIntelligence(id: string): Promise<AccountIntelligenceDTO | null> {
  ...
  const [signals, opportunities, contacts, notes, tasks, activities, outreachDrafts] = await Promise.all([
    pool.query<{
      ...
      `SELECT s.id::text, s.title, s.category, s.summary, s.status::text, s.lifecycle_stage,
              ...
              s.power_capacity_mw::text, s.investment_usd_millions::text,
    ...
    pool.query<{ id:string;full_name:string;job_title:string|null;email:string|null;phone:string|null;stakeholder_role:AccountIntelligenceDTO["contacts"][number]["stakeholderRole"];engagement_status:AccountIntelligenceDTO["contacts"][number]["engagementStatus"];last_contacted_at:Date|null;next_follow_up_at:Date|null;created_at:Date }>("SELECT id::text,full_name,job_title,email,phone,stakeholder_role,engagement_status,last_contacted_at,next_follow_up_at,created_at FROM account_contacts WHERE account_id=$1 ORDER BY is_primary DESC,created_at DESC",[id]),
    ...
    pool.query<{
      ...
      `SELECT id::text, subject, body, status, generated_by_email, created_at, sent_at
  ...
  return {
    ...
    signals: signals.rows.map((signal) => ({
      ...
      powerCapacityMw: signal.power_capacity_mw === null ? null : Number(signal.power_capacity_mw),
      investmentUsdMillions: signal.investment_usd_millions === null ? null : Number(signal.investment_usd_millions),
    ...
    outreachDrafts: outreachDrafts.rows.map((draft) => ({
      ...
      sentAt: draft.sent_at?.toISOString() ?? null,
    })),
    ...
    contacts: contacts.rows.map((contact)=>({id:contact.id,fullName:contact.full_name,jobTitle:contact.job_title,email:contact.email,phone:contact.phone,stakeholderRole:contact.stakeholder_role,engagementStatus:contact.engagement_status,lastContactedAt:contact.last_contacted_at?.toISOString()??null,nextFollowUpAt:contact.next_follow_up_at?.toISOString()??null,createdAt:contact.created_at.toISOString()})),
